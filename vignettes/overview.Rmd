...
In an experimental study, once the metabolite abundances 
are determined it is _usually_ feasible to elaborate 
a list of affected metabolites. This list, that may vary in size, 
...
a list of prioritised pathways helps identify 
...
the top ranked pathways. The priorisation of the pathways 
and other entiteis stems from a 
holistic diffusion process on a __graph representation__ 
...
This is stored in a unique `FELLA.USER` S4 object, along with user analyses.
...
p-values are computed through z-scores 
...
p-values are computed through Monte Carlo trials 
of the random variables. 
This approximation requires computing the random trials, 
governed by the `ntrials` argument. 

Vignettes are long form documentation 
commonly included in packages.
Because they are part of the distribution of the package, 
they need to be as compact as possible. 
The `html_vignette` output type provides 
a custom style sheet (and tweaks some options) 
to ensure that the resulting html is 
...
at once with the option `method = "all"`:
...
The option `splitByConnectedComponent` plots the graph 
connected component-wise, which can be displayed 
by pressing the enter key if the argument 
`askPlots` is set to `TRUE`. 
For the small example in this vignette, 
the subgraph will be plotted as a whole.
