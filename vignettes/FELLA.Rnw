...
\author[3]{Francesc Fern\'andez-Albert}
\author[4,5,6]{Maria Vinaixa}
\author[4,5]{Oscar Yanes}
...
\affil[3]{Boehringer Ingelheim Pharmaceuticals Inc, Germany}
\affil[4]{Centre for Omic Sciences, Department of Electronic Engineering, 
Rovira i Virgili University, Reus, 43204, Spain}
\affil[5]{Metabolomics Platform, Spanish Biomedical Research Centre in 
Diabetes and Associated Metabolic Disorders, Madrid, 28029, Spain}
\affil[6]{Manchester Synthetic Biology Research Centre for Fine and 
...
\begin{figure}[!tpb]
\centerline{\includegraphics[width=.8\linewidth]{HD_contour.pdf}}
...
\begin{figure}[!tpb]
\centerline{\includegraphics[width=.8\linewidth]{PR_contour.pdf}}
...
\item Compounds upload: contains a general description of 
...
\item Advanced options: widgets that contain the main 
...
\item Results: interactive plot with the sub-graph with the 
...
\item Export: several tabular and network exporting options.
...
[TEC2014-60337-R to A.P.].
A.P. and S.P. thank for funding the
