...
<<0a_exporting_helper, echo=FALSE, eval=TRUE>>=
# Options for generating supplementary files?
# If set to FALSE chunks starting with 0a_* will be ignored
flag.supplement <- TRUE
dir.out <- "~/fella_sm"

# If you want to generate the supplementary files, 
# do not knit the vignette but run all its chunks in Rstudio
# 
# All the chunks related to the supplement start with 0a_*
# They can be left as eval=FALSE without affecting the main vignette 
# 
# This function exports the results to report them as supplementary files
export_for_supplementary <- function(
    dir.out, # folder to save files
    prefix, # dataset prefix
    fella.user, fella.data, nlimit, # arguments for FELLA
    ...) {
    ...
    # Export input
    file_names <- paste0(dir.out, "/", prefix, "_input.csv")
    df_names <- getInput(fella.user) %>%
        getName(data = fella.data) %>% 
        plyr::ldply(function(x) data.frame(
            Names = paste(x, collapse = ",")), .id = "KEGG id")
    utils::write.csv(df_names, file = file_names, row.names = FALSE)
    
    # Export table
    file_results <- paste0(dir.out, "/", prefix, "_subgraph.csv") 
    exportResults(
        format = "csv", 
        file = file_results, 
        object = fella.user, 
    ...
    # Export plot
    file_graph <- paste0(dir.out, "/", prefix, "_subgraph.pdf") 
    plot_title <- paste0("Nodes reported in the '", prefix, "' dataset ")
    plot_sub <- paste0("Node limit: ", nlimit)
    grDevices::pdf(file_graph, width = 7, height = 7)
    plot(
        fella.user, 
        data = fella.data, 
        nlimit = nlimit, 
        ...)
    graphics::title(main = plot_title, sub = plot_sub)
    grDevices::dev.off()
}
@
...
In this example, no matrices will be loaded.
...
<<0a_03_export_supplementary, echo=FALSE, eval=TRUE>>=
if (flag.supplement) {
    export_for_supplementary(
        dir.out = dir.out, 
        prefix = "epithelial", 
        fella.user = analysis.epithelial, 
        fella.data = fella.data, 
        method = "diffusion", 
        nlimit = nlimit, 
        vertex.label.cex = vertex.label.cex
    )
...
<<0a_05_export_supplementary, echo=FALSE, eval=TRUE>>=
if (flag.supplement) {
    export_for_supplementary(
        dir.out = dir.out, 
        prefix = "ovarian", 
        fella.user = analysis.ovarian, 
        fella.data = fella.data, 
        method = "diffusion", 
        nlimit = nlimit, 
        vertex.label.cex = vertex.label.cex)
...
<<0a_06_export_supplementary, echo=FALSE, eval=TRUE>>=
if (flag.supplement) {
    export_for_supplementary(
        dir.out = dir.out, 
        prefix = "malaria", 
        fella.user = analysis.malaria, 
        fella.data = fella.data, 
        method = "diffusion", 
        nlimit = nlimit, 
        vertex.label.cex = vertex.label.cex
...
<<0a_check_validation, echo=FALSE, eval=TRUE>>=
if (flag.supplement) {
    # Number of metabolites per study
    list_datasets <- c("epithelial", "ovarian", "malaria")
    df_metabolites <- plyr::ldply(
        list_datasets, function(dataset) {
            input <- get(paste0("compounds.", dataset))
            mapped <- getInput(get(paste0("analysis.", dataset)))
    ...
    write.csv(
        df_metabolites, file = paste0(dir.out, "/descriptive_input.csv"), 
        row.names = FALSE)
    
    # See if all the entities mentioned in the vignette apear in their solutions
    df_val <- utils::read.csv("validation.csv")
    
    df_test <- plyr::ddply(
        df_val, "dataset", function(df_db) {
            dataset <- as.character(df_db$dataset[1])
            df_tmp <- utils::read.csv(
                paste0(dir.out, "/", dataset, "_subgraph.csv"))
            df_db$in_solution <- df_db$KEGG.id %in% df_tmp$KEGG.id
            df_db
        }
    )
    utils::write.csv(
        df_test, file = paste0(dir.out, "/descriptive_hits.csv"), 
        row.names = FALSE)
    
    # Write session info for supplement as well
    writeLines(
        capture.output(sessionInfo()), 
        con = paste0(dir.out, "/info_session.txt"))
    writeLines(
        getInfo(fella.data), 
        con = paste0(dir.out, "/info_kegg.txt"))
    writeLines(
        capture.output(print(fella.data)), 
        con = paste0(dir.out, "/info_fella_data.txt"))
    
    # Save database
    save(
        fella.data, 
        file = paste0(dir.out, "/fella_data.RData"), 
        compress = "xz")
    # Zip the files
    # utils::zip(zipfile = paste0(dir.out, ".zip"), files = dir.out)
    # unlink("suppl", recursive = TRUE)
}
@
