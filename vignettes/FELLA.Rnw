...
write.csv(
    df_metabolites, file = "suppl/descriptive_input.csv", row.names = FALSE)

# See if all the entities mentioned in the vignette apear in their solutions
df_val <- utils::read.csv("vignettes/validation.csv")

df_test <- plyr::ddply(
    df_val, "dataset", function(df_db) {
        dataset <- as.character(df_db$dataset[1])
        df_tmp <- utils::read.csv(paste0("suppl/", dataset, "_subgraph.csv"))
        df_db$in_solution <- df_db$KEGG.id %in% df_tmp$KEGG.id
        df_db
    }
)
utils::write.csv(
    df_test, file = "suppl/descriptive_hits.csv", row.names = FALSE)

# Write session info for supplement as well
writeLines(capture.output(sessionInfo()), con = "suppl/info_session.txt")
writeLines(getInfo(fella.data), con = "suppl/info_kegg.txt")

# Zip the files
utils::zip(zipfile = "suppl.zip", files = "suppl")
